  ...
  <div @scroll="onScroll">
    ...
    <div class="main-content">
      ...
      <div class="content-area">
        ...
        <div v-for="draft in drafts" class="card">
          <!--  -->
          <RouterLink :to="{ name: 'new', params: { id: draft.id } }">
            <h2>{{ draft.title }}</h2>
          </RouterLink>
          <p>{{ draft.title }}...</p>

          <button v-if="!loading" @click="submit(draft.id)">提交</button>
        </div>
...
export default {
  ...
  methods: {
    ...
    async getDraft() {
      this.loading = true;
      try {
        const response = await axios.get('/api/articles/draftsOfAuthor', {
          params: {
            authorId: 101,
          }
        });
    ...
    async submit(articleId) {
      console.log(this.commentContent);
      const response = await axios.put('/api/articles/publishArticles?articleId=' + articleId);
      this.getDraft();
    },
